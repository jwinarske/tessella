...
permutation key, composite (zoom-interpolated) binding and the line layer's uniform buffers
land; the probe gains `--zoom` and two further goldens; DR-19 gains the line-path confirmation that the rotation is wagyu's
...
- **R1** — *in progress.* Vector tiles decode and tessellate (MVT 2.1, no dependency); the line
  ...
  buffers land, and the frame emits twelve of the oracle's fourteen — all but the circle
  layer's. Remaining: network + cache with request coalescing, cross-faded (pattern) binders,
  the circle layer, DR-11 evaluator, §12.5 startup path. Exit: probe parity on a real style
  sans symbols; cold-boot-to-first-tile traced and within budget.
