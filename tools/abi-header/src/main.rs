...
/// The same numbers, for a consumer that reads the stream out of a `WebAssembly.Memory`.
///
/// A second emitter rather than a hand-written table. JavaScript has no `offsetof` and no
/// `static_assert`, so a consumer that wrote its own offsets would be the one description of the
/// stream that nothing checks -- which is the drift this whole generator exists to prevent, and
/// it would arrive in the one language where the failure is a silent misread rather than a
/// compile error.
const JS_OUTPUT: &str = "web/abi.js";
...
fn main() -> ExitCode {
    ...
    if check {
        let mut stale = false;
        for (name, path, generated) in &outputs {
            let current = std::fs::read_to_string(path).unwrap_or_default();
            if current == *generated {
                println!("{name} is up to date");
            } else {
                eprintln!("{name} does not match the Rust definitions; re-run without --check");
                stale = true;
            }
        }
        return if stale {
...
/// The JavaScript view: every size, alignment and offset the C header asserts, as data.
///
/// Only the numbers. A JS consumer reads the stream with a `DataView`, so what it needs from the
/// ABI is where each field is and how wide the struct is -- not declarations, which it has no way
/// to check anyway.
fn generate_js() -> String {
    ...
    writeln!(
        w,
        "// The tessella capture-stream ABI, revision {ABI_REV}, as offsets."
    )
    .unwrap();
    writeln!(w, "//").unwrap();
    for line in wrap(
        "The Rust definitions in tessella-capture-abi are the source of truth, and these numbers          came from `size_of`, `align_of` and `offset_of!` on them at generation time -- the same          call that wrote the C header's static assertions. A consumer reading the ring out of a          `WebAssembly.Memory` has no compiler to check its arithmetic, so this is the only thing          standing between it and a silent misread.",
        94,
    ) {
    ...
    writeln!(w, "export const ABI_REV = {ABI_REV};").unwrap();
    writeln!(w, "export const RECORD_ALIGN = {RECORD_ALIGN};").unwrap();
    writeln!(w, "export const PAYLOAD_ALIGN = {PAYLOAD_ALIGN};").unwrap();
    writeln!(w, "export const RECORD_FLAG_SKIP = {RECORD_FLAG_SKIP};").unwrap();
    writeln!(w).unwrap();

    writeln!(w, "/** Envelope kinds carried on the ring. */").unwrap();
    writeln!(w, "export const KIND = Object.freeze({{").unwrap();
    ...
    .unwrap();
    writeln!(w, "export const LAYOUT = Object.freeze({{").unwrap();
    for s in structs() {
        writeln!(w, "  {}: {{", s.c_name).unwrap();
        writeln!(w, "    size: {}, align: {},", s.size, s.align).unwrap();
        writeln!(w, "    at: {{").unwrap();
