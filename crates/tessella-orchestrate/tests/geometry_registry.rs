...
fn a_tile_that_stays_keeps_its_id() {
    ...
    let first: Vec<_> = [1, 2, 3]
        .map(|x| registry.id_for(key(x, 0, 1), [0, 0, 0]))
        .to_vec();
...
fn sub_layers_are_separate_drawables() {
    let mut registry = GeometryRegistry::new();
    registry.begin_frame(ViewId(0));
    let triangles = registry.id_for(key(1, 0, 1), [0, 0, 0]);
    let outline = registry.id_for(key(1, 0, 2), [0, 0, 0]);
...
fn what_a_frame_stops_using_is_reported_before_it_is_dropped() {
    let mut registry = GeometryRegistry::new();
    registry.begin_frame(ViewId(0));
    let leaving = registry.id_for(key(1, 0, 1), [0, 0, 0]);
    registry.id_for(key(2, 0, 1), [0, 0, 0]);
...
fn an_id_is_never_reused() {
    let mut registry = GeometryRegistry::new();
    registry.begin_frame(ViewId(0));
    let first = registry.id_for(key(1, 0, 1), [0, 0, 0]);
    registry.begin_frame(ViewId(0));
    registry.retire();
    assert!(registry.is_empty());

    registry.begin_frame(ViewId(0));
    let second = registry.id_for(key(9, 0, 1), [0, 0, 0]);
    assert_ne!(first, second, "a retired id is not handed out again");

    // Not even to the very drawable that had it.
    registry.begin_frame(ViewId(0));
    let again = registry.id_for(key(1, 0, 1), [0, 0, 0]);
...
fn a_viewport_drawable_has_a_key_too() {
    ...
    let tiled = registry.id_for(key(1, 0, 0), [0, 0, 0]);
...
mod views {
    ...
    fn a_shared_drawable_is_announced_once_and_bound_twice() {
        ...
        let id = registry.id_for(shared, [0, 0, 0]);
        registry.retire();
    ...
    fn one_views_frame_does_not_retire_anothers() {
        ...
        registry.begin_frame(ViewId(0));
        registry.id_for(key(1, 0, 1), [0, 0, 0]);
        registry.id_for(key(2, 0, 1), [0, 0, 0]);
        registry.retire();

        // A different cover entirely.
        registry.begin_frame(ViewId(1));
        registry.id_for(key(8, 0, 1), [0, 0, 0]);
        registry.id_for(key(9, 0, 1), [0, 0, 0]);
        assert!(
